...
def main() -> None:
    ...
    sections = [
        ...
        ("rtp.conf in container", "docker compose exec -T asterisk-a grep -E 'external_media|localnet|10.1.5' /etc/asterisk/rtp.conf | head -20"),
        ("rtp_ice_extra", "docker compose exec -T asterisk-a head -15 /etc/asterisk/rtp_ice_extra.conf"),
        ("pjsip_provider", "docker compose exec -T asterisk-a head -28 /etc/asterisk/pjsip_provider.conf"),
        ...
        ("coturn", "docker compose ps coturn 2>/dev/null; ss -ulnp | grep 3478 | head -3"),
