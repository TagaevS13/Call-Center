...
```mermaid
flowchart LR
  GSM["GSM UMG 10.1.5.64/27"]
  SIG["GSM SoftX 10.1.5.8/29"]
  subgraph PBX["PBX project"]
    NICsig["enp13s4f0 172.16.4.19 SIP"]
    NICrtp["enp6s0f0 172.16.6.183 RTP"]
    AST["Asterisk bridge"]
  end
  BR["Браузер агента 192.168.1.x"]
  SIG --> NICsig --> AST
  GSM <-->|"плечо 1 RTP"| NICrtp
  NICrtp --> AST
  AST <-->|"плечо 2"| NICrtp
  NICrtp <-->|"WSS:8089 / RTP-DTLS / TURN:3478"| BR
```

> **Быстрая локализация одной командой** (на сервере, во время тестового звонка):
> `bash scripts/run-gsm-voice-diagnosis.sh 8` или `bash scripts/diag-audio.sh -w 8` — см. [gsm-voice-diagnosis.md](gsm-voice-diagnosis.md).

Два независимых плеча. Любое одно сломанное плечо = «не слышно» с одной стороны:

| Плечо | Симптом one-way | Где чинить |
| ----- | --------------- | ---------- |
| 1. GSM -> Asterisk | агент **не слышит абонента**; в логах нет `Got RTP ... 10.1.5` (любой хост в `10.1.5.64/27`) | маршрут/`rp_filter` на хосте + **UMG/SBC не шлёт RTP на 172.16.6.183** |
...
- `pjsip_provider`: `media_address=172.16.6.183`, `bind_rtp_to_media_address=yes`, `match=10.1.5.8/29` и `10.1.5.64/27`.
...
# Маршруты — split: 10.1.5.8/29 via 172.16.4.1 enp13s4f0, 10.1.5.64/27 via 172.16.6.131 enp6s0f0
ip r | grep 10.1.5
bash scripts/verify-gsm-config.sh
cat /proc/sys/net/ipv4/conf/enp13s4f0/rp_filter
cat /proc/sys/net/ipv4/conf/enp6s0f0/rp_filter
```

Во время тестового звонка:

```bash
sudo tcpdump -ni enp13s4f0 host 10.1.5.10 and port 5060 -vv -c 10
sudo tcpdump -ni enp6s0f0 net 10.1.5.64/27 and udp -vv -c 40
sudo tcpdump -ni enp6s0f0 src net 10.1.5.64/27 and udp -c 5
```

- SIG на **enp13s4f0**, RTP на **enp6s0f0** — **In и Out** на медиа-интерфейсе.
...
> (любой хост UMG внутри `/27` — в SDP `c=` бывает `10.1.5.72`, `10.1.5.75` и т.д., не один фиксированный IP)
> на наш PBX **`172.16.6.183`**, UDP, порты RTP согласно SDP (обычно 10000–20000).
> Сейчас мы шлём RTP в вашу сторону (симметричный RTP, `rtp_symmetric=yes`), но обратного потока
> **`10.1.5.64/27 → 172.16.6.183`** на PBX нет (tcpdump `src net 10.1.5.64/27`: 0 пакетов).
...
systemctl restart cc-asterisk-prestart cc-asterisk cc-webui          # пересоздаст оба (Asterisk-профиль + браузерный bundlePolicy)
```
...
3. Если стало лучше — оставить `standard`. Если хуже (Chrome/Edge теряют inbound при
   `bundle=yes`) — вернуть `AGENT_WEBRTC_MODE=manual` и `systemctl restart cc-asterisk-prestart cc-asterisk cc-webui`.
